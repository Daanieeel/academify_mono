...
const ThemedUserBadge = (props: ThemedUserBadgeProps) => {
    ...
    return (
        <View style={{
            ...
            <IcomoonIcon name={props.icomoonIconName} color={primary100Color}></IcomoonIcon>
            <ThemedText type='caption' color={primary100Color}>{uppercaseLabel}</ThemedText>
        </View>
    )
}
...
const ThemedProfilePreview = ({ firstName = '', lastName = '', ...props }: ThemedProfilePreviewProps) => {
    ...
    return (
        <View style={[styles['main-container'], { backgroundColor: neutral100Color }]}>
            <ProfilePic customBorderColor={neutral400Color} size='large' source={props.imageSource}></ProfilePic>
            <View style={{
                gap: 0,
                alignItems: 'center'
            }}>
                <ThemedText type='subHeading'>{firstName + ' ' + lastName}</ThemedText>
                <ThemedText color={neutral700Color} type='caption'>{props.username}</ThemedText>
            </View>
            <View style={{
                flexDirection: 'row',
                gap: 5
