...
const ThemedSlider = ({ currentOption = 0, ...props }: ThemedSliderProps) => {
    ...
    const translateX = useRef(new Animated.Value((selectedContainerWidth) * currentOption)).current
                ...
                props.options.map((optionName, index) =>
                    ...
                    >
                        <View key={index} >
                            <ThemedText
                                numberOfLines={1}
                                color={index == currentOption ? selectedTextColor : deselectedTextColor}
                                type='body'>
                                {optionName}
                            </ThemedText>
                        </View>
