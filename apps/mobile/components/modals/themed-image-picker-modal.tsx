...
const ThemedImagePickerModal = (props: ThemedImagePickerModalProps) => {
    ...
    const profilePicArray: React.ReactNode[] = [
        ...IMAGE_URIS.map((uri) => <ProfilePic onPress={() => onAvatarPressed(uri)} avatarType='group' size='large' source={uri}></ProfilePic>),
        <ProfilePic onPress={() => onAvatarPressed(undefined)} avatarType='group' size='large' source={undefined} icomoonIcon='prohibit'></ProfilePic>
    ]
